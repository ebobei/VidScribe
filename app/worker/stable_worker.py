"""Stable one-by-one worker for overnight VidScribe collection."""
...
class StableWorker:
    """Processes accepted videos slowly and persists state after every attempt."""
    ...
    def run(self) -> list[CandidateDecision]:
        ...
        for row_position, (queue_index, decision, attempt_count) in enumerate(rows, start=1):
            if decision.status != VideoStatus.PROCESSED:
                logger.info(
                    "Skipping queue item %s/%s video_id=%s status=%s reason=%s",
                    row_position,
                    total,
            ...
            if is_fully_processed(decision):
                logger.info(
                    "Already processed queue item %s/%s video_id=%s",
                    row_position,
                    total,
                    decision.candidate.video_id,
                )
            ...
            if row_position < total:
                sleep_seconds = random_video_sleep(
                    self.config.stability.video_sleep_min_seconds,
                    self.config.stability.video_sleep_max_seconds,
                )
                if sleep_seconds > 0:
    ...
    def _process_with_retries(
        ...
        while attempt_count < max_attempts:
            ...
            logger.info(
                "Processing video queue_index=%s video_id=%s attempt=%s/%s",
                queue_index,
                current.candidate.video_id,
                attempt_number,
                max_attempts,
            )
            ...
            try:
                ...
                self.store.record_attempt(
                    ...
                    video_id=current.candidate.video_id,
                    step="subtitle_pipeline",
                ...
                self.store.update_decision(
                    queue_index,
                    mark_failed(current, reason, message),
                    attempt_count=attempt_number,
                    last_error=message,
                )
                ...
                if rate_limited and attempt_number < max_attempts:
                    ...
                    logger.warning(
                        "Rate limit detected for video_id=%s. Sleeping %s seconds before retry.",
                        current.candidate.video_id,
                        sleep_seconds,
                    )
                    if sleep_seconds > 0:
                        time.sleep(sleep_seconds)
                    # Reset current to retryable accepted state for the next attempt.
                ...
                if not rate_limited and attempt_number < max_attempts:
                    ...
                    logger.warning(
                        "Video processing failed for video_id=%s. Sleeping %s seconds before retry.",
                        current.candidate.video_id,
                        sleep_seconds,
                    )
                    if sleep_seconds > 0:
    ...
    def _process_once(self, queue_index: int, decision: CandidateDecision) -> CandidateDecision:
        ...
        try:
            subtitle_info = self.downloader.download(
                index=queue_index,
                candidate=decision.candidate,
                selection=selection,
                run_root=self.paths.root,
                subtitles_raw_dir=self.paths.subtitles_raw_dir,
            )
        ...
        updated_subtitle = subtitle_info.model_copy(
            update={"clean_transcript_path": result.clean_transcript_path.relative_to(self.paths.root).as_posix()}
        )
