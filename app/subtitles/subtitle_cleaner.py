"""Subtitle cleaning for VidScribe Stage 4.

The cleaner converts raw .vtt/.srt subtitle files into plain text transcripts.
It intentionally does not call external services and does not perform speech
recognition. Stage 4 output is a cleaned .txt file per downloaded subtitle.
"""
...
class SubtitleCleaningError(Exception):
    """Raised when a raw subtitle file cannot be cleaned into text."""


@dataclass(frozen=True)
class CleanTranscriptResult:
    """Result of cleaning one subtitle file."""
...
class SubtitleCleaner:
    """Cleans .vtt/.srt subtitle files into readable plain text."""
    ...
    def _build_plain_text(self, lines: list[str]) -> str:
        # Subtitle cues are often tiny fragments. Join them into a readable text,
        # but keep deterministic wrapping so the output remains diff-friendly.
        joined = " ".join(lines)
