"""Subtitle availability selection for VidScribe Stage 3.

The selector does not download anything. It inspects yt-dlp metadata and picks
one subtitle track according to the MVP priority:

1. manual subtitles for the first preferred language;
2. auto subtitles for the first preferred language;
3. manual subtitles for the next preferred language;
4. auto subtitles for the next preferred language.
"""
...
class SubtitleSelection:
    """Selected subtitle track from yt-dlp metadata."""
...
class SubtitleSelector:
    """Selects the best subtitle track available for a video."""
    ...
    def select(self, raw_metadata: dict[str, Any], config: RunConfig) -> SubtitleSelection | None:
        ...
        for requested_language in config.languages.preferred:
            if config.languages.allow_manual_subtitles:
                selected = self._select_from_mapping(
                    mapping=manual_subtitles,
...
def _find_language_key(mapping: dict[str, list[dict[str, Any]]], requested_language: str) -> str | None:
    ...
    for language in mapping.keys():
        ...
        if (
            language_lower.startswith(f"{requested}-")
            or language_lower.startswith(f"{requested}_")
            or language_lower.startswith(f"{requested}.")
        ):
    ...
    # Prefer the shortest variant, e.g. en-US before en-US-orig-long-name.
