"""yt-dlp subtitle downloader for VidScribe Stage 3.

This module is intentionally narrow: it downloads only one selected subtitle
track per video and always includes --skip-download. It never downloads video
or audio files.
"""
...
class YtDlpSubtitleError(Exception):
    """Base error for subtitle downloading failures."""


class YtDlpSubtitleNoFileError(YtDlpSubtitleError):
    """Raised when yt-dlp finished but did not create a subtitle file."""


class YtDlpSubtitleDownloader:
    """Downloads one selected subtitle file for a video via yt-dlp."""
    ...
    def download(
        ...
        *,
        ...
        logger.info(
            "Downloading subtitle: video_id=%s language=%s type=%s format=%s",
            candidate.video_id,
            selection.selected_language,
            selection.subtitle_type.value,
            selection.subtitle_format,
        )
        ...
        if completed.returncode != 0:
            ...
            if "No module named yt_dlp" in stderr or "No module named yt-dlp" in stderr:
                raise YtDlpSubtitleError(
                    "yt-dlp is not installed in the active Python environment. "
                    "Run: python -m pip install -r requirements.txt"
                )
            raise YtDlpSubtitleError(
                f"yt-dlp subtitle download failed for {candidate.video_id} "
                f"with exit code {completed.returncode}: {stderr}"
            )

        downloaded_file = self._find_downloaded_subtitle(temp_dir)
        if downloaded_file is None:
            raise YtDlpSubtitleNoFileError(
                f"yt-dlp did not create a subtitle file for {candidate.video_id} "
                f"({selection.selected_language}, {selection.subtitle_type.value})"
            )

        final_path = self._move_to_final_path(
            index=index,
            candidate=candidate,
            selection=selection,
            source_path=downloaded_file,
            subtitles_raw_dir=subtitles_raw_dir,
        )
        self._cleanup_temp_dir(temp_dir)

        return SubtitleInfo(
            video_id=candidate.video_id,
            selected_language=selection.selected_language,
            subtitle_type=selection.subtitle_type,
            subtitle_format=final_path.suffix.lstrip(".").lower(),
            raw_subtitle_path=final_path.relative_to(run_root).as_posix(),
    ...
    def _build_command(self, url: str, selection: SubtitleSelection, temp_dir: Path) -> list[str]:
        command = [
            ...
            selection.selected_language,
            "--sub-format",
            f"{selection.subtitle_format}/best",
            "--paths",
            str(temp_dir),
            "--output",
            "%(id)s.%(ext)s",
    ...
    def _cleanup_temp_dir(self, temp_dir: Path) -> None:
        ...
        try:
            ...
            # Other temporary video directories may still exist in the same run.
            pass
