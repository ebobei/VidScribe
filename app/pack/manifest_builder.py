...
def build_manifest(*, config: RunConfig, paths: RunPaths, created_at: datetime, decisions: list[CandidateDecision], artifacts: list[RunArtifact], transcript_documents: list[TranscriptDocument], chunks: list[RagChunk]) -> dict[str, Any]:
    ...
    return {
        ...
        'state_store': 'SQLite state.sqlite' if config.stability.enabled else None,
        'search_note': 'VidScribe uses local yt-dlp search/extraction instead of Google Cloud or YouTube Data API. Video and audio files are not downloaded. Subtitle commands always use --skip-download. Stable mode uses flat discovery, SQLite state, one-by-one metadata extraction, pauses, retry, partial result packaging and explicit unavailable/age-restricted statuses.',
        'config': config.model_dump(mode='json'),
        'chunking': {
            'enabled': config.output.build_chunks_jsonl,
            'chunk_size_words': 1000,
            'overlap_words': 120,
            'timestamps_note': 'Chunk start_time/end_time are best-effort values derived from raw subtitle cue timings. They may be null when timing data cannot be parsed.',
        },
