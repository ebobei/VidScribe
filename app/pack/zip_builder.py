...
def _iter_pack_files(paths: RunPaths) -> list[Path]:
    ...
    for file_path in [paths.run_json, paths.manifest_json, paths.videos_csv, paths.summary_input_md, paths.chunks_jsonl, paths.collect_log]:
        if file_path.exists() and file_path.is_file():
            candidates.append(file_path)
    for directory in [paths.metadata_dir, paths.subtitles_raw_dir, paths.transcripts_clean_dir, paths.chunks_dir, paths.logs_dir]:
