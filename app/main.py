...
Stage 7 adds a local stability mode for overnight collection:
SQLite state, one slow worker, retry/backoff, resume and partial pack building.
The project remains a local CLI utility without Redis, server, web UI or DB server.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    collect.add_argument(
        ...
        help="Override YouTube search order. relevance/date are supported best; others are best-effort.",
    )
    collect.add_argument("--output-directory", help="Override output.directory from YAML config.")
    collect.add_argument(
        "--stable",
        action="store_true",
        help="Enable stable overnight mode: SQLite state, one-by-one processing, pauses and retry/backoff.",
    )
    collect.add_argument("--verbose", action="store_true", help="Enable verbose logging.")

    resume = subparsers.add_parser(
        "resume",
        help="Resume a stable run from an existing output run directory.",
    )
    ...
    pack = subparsers.add_parser(
        "pack",
        help="Rebuild videos.csv, summary_input.md, manifest.json and research_pack.zip from an existing stable run.",
    )
...
def build_stage7_artifacts(paths: RunPaths, config: RunConfig) -> list[RunArtifact]:
    artifacts = [
        RunArtifact(name="run.json", path=paths.relative_to_root(paths.run_json), description="Resolved run metadata and counters."),
        RunArtifact(name="manifest.json", path=paths.relative_to_root(paths.manifest_json), description="Main manifest with query, counters, videos, transcript info and chunk info."),
        RunArtifact(name="videos.csv", path=paths.relative_to_root(paths.videos_csv), description="Video candidates with processed/skipped/failed statuses and reasons."),
        RunArtifact(name="state.sqlite", path=paths.relative_to_root(paths.state_db), description="SQLite state for stable runs, resume and attempt history."),
        RunArtifact(name="metadata/", path=paths.relative_to_root(paths.metadata_dir), description="Raw yt-dlp metadata JSON files for videos with downloaded subtitles."),
        RunArtifact(name="subtitles_raw/", path=paths.relative_to_root(paths.subtitles_raw_dir), description="Raw selected subtitle files downloaded through yt-dlp with --skip-download."),
        RunArtifact(name="transcripts_clean/", path=paths.relative_to_root(paths.transcripts_clean_dir), description="Cleaned plain-text transcripts generated from raw subtitle files."),
        RunArtifact(name="chunks/chunks.jsonl", path=paths.relative_to_root(paths.chunks_jsonl), description="RAG-ready JSONL chunks generated from clean transcripts."),
    ]

    if config.output.build_summary_md:
        artifacts.append(
            RunArtifact(name="summary_input.md", path=paths.relative_to_root(paths.summary_input_md), description="Human-readable research input file with run context, video table and clean transcripts.")
        )
...
def count_decisions(decisions: list[CandidateDecision]) -> dict[str, int]:
    return {
        ...
        "clean_transcripts_count": sum(1 for item in decisions if item.subtitle_info is not None and item.subtitle_info.clean_transcript_path),
    }
    ...
    counters.update(
        {
            ...
            "summary_input_built": 1 if config.output.build_summary_md else 0,
            "research_pack_built": 1 if config.output.build_zip else 0,
        }
    )
...
def run_collect(args: argparse.Namespace) -> int:
    ...
    if config.limit > 20 or config.candidate_pool_size > 60:
        logger.warning(
            "Large collection detected: limit=%s candidate_pool_size=%s. Stable mode is recommended.",
            config.limit,
    ...
    if config.stability.enabled:
        logger.info(
            "Stage 7 stable mode enabled: SQLite state=%s, sleep=%s-%s sec, 429 backoff=%s sec, max_attempts=%s",
            paths.state_db,
            config.stability.video_sleep_min_seconds,
            config.stability.video_sleep_max_seconds,
            config.stability.rate_limit_backoff_seconds,
            config.stability.max_attempts_per_video,
        )
    ...
    counters = build_final_artifacts(
        config=config,
        paths=paths,
        decisions=decisions,
        created_at=created_at,
...
def run_resume(args: argparse.Namespace) -> int:
    ...
    counters = build_final_artifacts(
        ...
        cli_overrides=dict(info.get("cli_overrides") or {}),
        status="completed",
    )
...
def run_pack(args: argparse.Namespace) -> int:
    ...
    counters = build_final_artifacts(
        ...
        config_path=str(info.get("config_path") or ""),
        cli_overrides=dict(info.get("cli_overrides") or {}),
        status="partial" if counters_have_failures(decisions) else "completed",
    )
    print_summary(paths, counters, stable=True)
    return 0
