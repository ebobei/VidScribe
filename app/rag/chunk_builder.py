"""RAG-ready chunk generation for VidScribe Stage 5+.

Stage 5+ stays intentionally small: it creates local JSONL chunks from cleaned
transcripts and, when possible, derives approximate chunk time ranges from the
raw subtitle file. It does not create embeddings and does not use a vector DB.
"""
...
class ChunkBuildError(VidScribeError):
    """Raised when chunk artifacts cannot be generated."""
...
class ChunkBuilder:
    """Builds TranscriptDocument metadata and RagChunk records."""
    ...
    _INLINE_VTT_TIMECODE_RE = re.compile(r"<\d{1,2}:\d{2}:\d{2}[,.]\d{3}>")
    _NOISE_TAG_RE = re.compile(
        r"\[(?:music|applause|laughter|silence|музыка|аплодисменты|смех|тишина)\]",
        flags=re.IGNORECASE,
    )
        ...
        *,
        ...
        for decision in decisions:
            ...
            document = TranscriptDocument(
                document_id=f"yt_{decision.candidate.video_id}",
                video_id=decision.candidate.video_id,
                title=decision.candidate.title,
                channel_title=decision.candidate.channel_title,
                url=decision.candidate.url,
                language=subtitle.selected_language,
                subtitle_type=subtitle.subtitle_type,
                text_path=subtitle.clean_transcript_path,
            ...
            chunks.extend(
                self._build_chunks_for_document(
                    document=document,
                    decision=decision,
                    tokens=tokens,
                )
            ...
            if text:
                result.append(
                    RagChunk(
                        ...
                        start_time=self._first_time(window, attr="start_time"),
                        end_time=self._last_time(window, attr="end_time"),
                        text=text,
